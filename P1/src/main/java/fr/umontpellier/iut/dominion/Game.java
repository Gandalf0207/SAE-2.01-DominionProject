...
public class Game {
    ...
    private ArrayList<String> flagCardNameGainCurseWhenDrawFromSupply = new ArrayList<>();

    private LastGainCardByLastPlayerWhoPlay lastPlayerGainCard = new LastGainCardByLastPlayerWhoPlay(currentTurnPlayer);

    private List<Embargo> affectedEmbargo = new ArrayList<>();

    boolean playAgain =false;

    public LastGainCardByLastPlayerWhoPlay getlastPlayerGainCard() {
        return lastPlayerGainCard;
    }
    ...
    public List<Card> getAvailableSupplyCards() {
        List<Card> cartesARetourner = new ArrayList<>();
        for(SupplyPile supplyPile : supplyPiles){
            if(!supplyPile.isEmpty()){
                Card c = supplyPile.getLast();
                cartesARetourner.add(c);
            }
        }
        /*return supplyPiles.stream()
                .filter(pile -> !pile.isEmpty())
                .map(pile -> pile.getLast())
                .toList();*/
    ...
    public void moveCardToSupply(Card c) {
        for (SupplyPile supplyPile : supplyPiles) {
            if(c.getName().equals(supplyPile.getName())) {
                c.moveTo(supplyPile);
            }
        }
    }
    ...
    /**
     * donne une carte passé en parametre à tout les adversaire depuis le supply
     * @param c
     * @param p
     * @param isAnAttack
     */
    public void distribuateAllOpponentsThisCard(Card c, Player p, boolean isAnAttack) {
        for (Player opponents : players) {
            if (!opponents.equals(p)) {
                if((isAnAttack && !PlayerHasLightHouse(opponents)) || !isAnAttack) {
                    Card card = getCardFromSupply(c.getName());
                    if(card != null) opponents.gainToDiscard(card);
                }
            }
        }
    }

    public void addFlagCardNameGainCurseWhenDrawFromSupply(String name) {
        flagCardNameGainCurseWhenDrawFromSupply.add(name);
    }

    public void removeFlagCardNameGainCurseWhenDrawFromSupply(String name) {
        flagCardNameGainCurseWhenDrawFromSupply.remove(name);
    }

    public ArrayList<String> getFlagCardNameGainCurseWhenDrawFromSupply() {
        return flagCardNameGainCurseWhenDrawFromSupply;
    }
    ...
    public void addAffectedEmbargo(Embargo c) {
        this.affectedEmbargo.add(c);
    }

    public List<Embargo> getAffectedEmbargo() {
        return affectedEmbargo;
    }

    public boolean PlayerHasLightHouse(Player p) {
        // ajout des curse pour blockade
        boolean hasLightHouse = false;
        for (Card c : p.getCardsInPlay()) {
            if(c.hasName("Lighthouse") && c.getEffectLaunch()) {
                hasLightHouse = true;
            }
        }
    ...
    public boolean isFinished() {
        ...
        for (SupplyPile pile : supplyPiles) {
            if(pile.isEmpty()) {
                cpt++;
                if(cpt >= 3 || pile.getName().equals("Province")) {
                    return true;
                }
            }
        }
    ...
    public void moveToNextPlayer() {
        if(!playAgain) {
            int actualIndex = players.indexOf(currentTurnPlayer);
            if(actualIndex == players.size() -1) {
                this.currentTurnPlayer = this.players.getFirst();
            }
            else {
                this.currentTurnPlayer = players.get(actualIndex+1);
            }
            this.turnNumber++;
        }
        else {
